...
JSONNET_FILES   ?= $(shell find . -type f -not -path './dependencies/*' -not -path './vendor/*' \( -name '*.*jsonnet' -or -name '*.libsonnet' \))
...
YAML_FILES      ?= $(shell find . -type f -not -path './dependencies/*' -not -path './vendor/*' \( -name '*.yaml' -or -name '*.yml' \))
